...
from environment.profiles.composition import RuntimeComposition
...
def test_nominal_pool_dimensions_and_surface_are_consistent() -> None:
    environment = load_environment_profile_json(ENVIRONMENT_PROFILE)
    bounds = environment.pool_boundary.bounds

    assert bounds == [0.0, 5.0, 0.0, 3.5, 0.0, 0.75]
    assert bounds[1] - bounds[0] == pytest.approx(5.0)
    assert bounds[3] - bounds[2] == pytest.approx(3.5)
    assert bounds[5] - bounds[4] == pytest.approx(0.75)
    assert environment.hydrodynamics.water_current_field_bounds == bounds
    assert environment.free_surface.surface_z == pytest.approx(bounds[5])
    assert environment.free_surface.sloshing_pool_bounds == bounds[:4]
    assert environment.free_surface.sloshing_water_depth == pytest.approx(0.75)
    assert environment.free_surface.buoyancy_scale == pytest.approx(1.0)
...
def test_composition_combines_environment_and_robot_sources() -> None:
    ...
    assert cfg.domain_randomization.thruster_command_delay_steps_range == [13, 13]
    assert cfg.domain_randomization.mass_range == [robot.model.mass_kg] * 2
    assert cfg.domain_randomization.volume_range == [robot.model.displaced_volume_m3] * 2
