    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    if logging_enabled:
                        ...
                        done_mask = dones > 0
                        columns = [cur_reward_sum, cur_episode_length]
                        columns.append(done_mask.to(dtype=torch.float32))
                        completed_episode_data.append(torch.stack(columns, dim=-1))

                        keep_mask = ~done_mask
                        cur_reward_sum *= keep_mask
                        cur_episode_length *= keep_mask

                # One fixed-shape device-to-host transfer replaces one dynamic
                # ``nonzero`` plus two to four transfers per environment step.
                if logging_enabled and completed_episode_data:
            ...
            if it == start_iter and logging_enabled:
                ...
                repositories = self._git_backed_repositories(
                    [*self.git_status_repos, str(project_root)]
                )
