"""Policy 3: target-heading and actual-motion alignment for an x-forward AUV."""
...
POLICY = TrackingRewardPolicy(
    name="policy_3",
    description="Geometrically gates commanded heading alignment by actual body-x motion alignment.",
    position_weight=3.0,
...
def compute_rewards(
    ...
    """Reward tracking only when command and actual motion both follow the nose."""
    ...
    target_alignment = body_x_alignment_score(target_lin_vel_b, rew_forward_min_speed, 1.0)
    actual_alignment = body_x_alignment_score(root_lin_vel_b, rew_forward_min_speed, 0.0)
    # When the command itself is stationary, actual direction is irrelevant.
    actual_alignment = torch.where(
        target_speed > rew_forward_min_speed,
        actual_alignment,
        torch.ones_like(actual_alignment),
    )
    coupled_alignment = torch.sqrt(torch.clamp(target_alignment * actual_alignment, min=0.0, max=1.0))
    rew_forward = rew_scale_forward * target_alignment
    rew_motion_alignment = rew_scale_motion_alignment * coupled_alignment
