...
# These aliases keep older checkpoints and saved env.yaml files evaluable.
# New experiments should use only policy_N names.
...
def canonical_tracking_reward_policy_name(name: str) -> str:
    """Resolve a selectable or legacy name to its canonical policy_N name."""

    normalized = str(name)
    if normalized == "custom":
        return normalized
    canonical = LEGACY_REWARD_POLICY_ALIASES.get(normalized, normalized)
    if canonical not in TRACKING_REWARD_POLICIES:
...
# Compatibility API for code and checkpoints created before policy_N naming.
TRACKING_REWARD_PROFILES = TRACKING_REWARD_POLICIES
TrackingRewardProfile = TrackingRewardPolicy
apply_tracking_reward_profile = apply_tracking_reward_policy
available_tracking_reward_profiles = available_tracking_reward_policies
