"""Policy 4: anti-chatter reward with normalized L1 control penalties."""
...
POLICY = TrackingRewardPolicy(
    name="policy_4",
    description="Adds mandatory actual-motion alignment and normalized L1 anti-chatter penalties.",
    position_weight=3.0,
    ...
    # Match policy_1 at the normalized command boundaries while retaining the
    # stronger near-zero slope of an L1 anti-chatter penalty.  policy_1 sums
    # eight squared actions, so the equivalent mean-L1 effort scale is 8*0.01.
    # A full -1 to +1 reversal has squared L2 norm 32; dividing the mean-L1
    # delta by two below bounds it by one, so the equivalent rate scale is
    # 32*0.05.
    action_weight=0.08,
...
def compute_rewards(
    ...
    """Compute policy_1 tracking terms with scale-independent L1 controls."""
    ...
    rew_action = -rew_scale_actions * torch.mean(torch.abs(actions), dim=1)
    rew_action_rate = -rew_scale_action_rate * 0.5 * torch.mean(torch.abs(actions - previous_actions), dim=1)
    return (
        rew_pos
        + rew_ang
        + rew_track_vel
