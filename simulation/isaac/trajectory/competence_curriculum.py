...
The Python training manager writes one JSON campaign specification and starts
this script in its own session. Training and both held-out evaluations are
separate child processes and survive terminal or editor disconnects.
"""
...
from simulation.isaac.trajectory.experiment import (  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON campaign specification written by simulation.isaac.training.",
    )
    ...
    spec = ExperimentSpec(
        ...
        rlpolicy_root=Path(spec_data["rlpolicy_root"]),
