"""Reusable train/eval utilities for trajectory experiments.

This module intentionally has no Isaac Sim imports. Python CLIs and optional
notebook clients can inspect runs, build commands, collect CSV metrics, and
plot results while Isaac Sim remains isolated in train/eval subprocesses.
"""
...
class _LazyDependency:
    """Load analysis-only dependencies when an analysis function needs them."""
...
np = _LazyDependency("numpy")
pd = _LazyDependency("pandas")
...
def build_eval_command(
    ...
    if request.environment_profile is not None:
        command.extend(["--environment_profile", str(request.environment_profile)])
    if request.domain_randomization_spec is not None:
...
def curriculum_eval_requests(
    ...
    *,
    ...
    common = dict(
        ...
        environment_profile=train_request.environment_profile,
