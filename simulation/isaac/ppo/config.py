...
class AUVTrajPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """PPO profile for the 16,384-environment trajectory curriculum.

    Thirty-two mini-batches keep each optimizer batch at 131,072 transitions.
    Five hundred iterations provide 2.097 billion transitions and reserve most
    updates for the full-difficulty trajectory stage. Learning rate stays
    fixed for every rollout, KL stops excessive PPO updates early, and a
    bounded controller selects the rate for the next rollout.
    """
    ...
    # simulation.isaac.training overrides its widths and input layout together.
