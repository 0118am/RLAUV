...
from ..domain_randomization.actuators import reset_actuators
from ..domain_randomization.battery import reset_battery
from ..domain_randomization.current import reset_current
from environment.profiles.features import domain_randomization_feature_enabled
from ..domain_randomization.hydrodynamics import reset_hydrodynamics
from ..domain_randomization.observations import reset_observation_transport
from ..domain_randomization.rigid_body import (
...
class AUVTrajectoryMixin:
    ...
    def _reset_domain(self, env_ids: Sequence[int]):
        ...
        payload_enabled = reset_rigid_body(self, env_ids_device)
        self._reset_disturbance_domain(env_ids_device)
        if payload_enabled:
            apply_payload_hydrodynamics(self, env_ids_device)
        self._reset_observation_domain(env_ids_device)
    ...
    def _log_domain_randomization_state(self) -> None:
        ...
        add_stats("observation_noise_std", self.observation_noise_std)
        add_stats("observation_delay_steps", self.observation_delay_steps)
    ...
    def _reset_disturbance_domain(self, env_ids: Sequence[int]) -> None:
        ...
        reset_current(self, env_ids_device, stage)
        reset_hydrodynamics(self, env_ids_device, stage)
        reset_actuators(self, env_ids_device, stage)
        reset_battery(self, env_ids_device, stage)
    ...
    def _reset_observation_domain(self, env_ids: Sequence[int]) -> None:
        if not isinstance(env_ids, torch.Tensor):
            ...
            env_ids_device = env_ids.to(device=self.device, dtype=torch.long)
        reset_observation_transport(self, env_ids_device)
