"""Kinematically bounded closed reference trajectories for the AUV task.

The environment keeps the task distribution and reset sampling logic, while
this module owns the reusable geometry and time re-parameterization.  All
functions are Torch-only so reset-time table construction remains deterministic
on the same device as the vector environment.
"""
...
# Explicit speed-controlled training commands.  The legacy single-axis sine
# (type 2) randomly chooses x/y/z and the legacy helix (type 3) is period
# driven, so neither can express the requested Cartesian training matrix
# unambiguously.
LATERAL_SINE = 8
VERTICAL_SINE = 9
SPATIAL_HELIX = 10
...
def evaluate_geometry(
    ...
    *,
    ...
    # This is a closed, breathing loop rather than a monotone spiral.  Using
    # ``cos(q)`` (instead of the legacy ``cos(q / 2)``) makes one requested
    # period one complete closed reference cycle.
